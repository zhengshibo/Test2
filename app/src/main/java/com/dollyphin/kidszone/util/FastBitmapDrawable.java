...
class FastBitmapDrawable extends Drawable {
    ...
    ///M. ALPS01990645, for two cores performance issue.
    static final long CLICK_FEEDBACK_LESS_DURATION = 300;
    ///M.
    ...
    public void setPressed(boolean pressed) {
        if (mPressed != pressed) {
            ...
            if (mPressed) {
                ///M. ALPS01990645, for two cores performance issue.
                long duration = CLICK_FEEDBACK_DURATION;

                KidsZoneLog.d(KidsZoneLog.KIDS_THEME_DEBUG, "setPressed,Processors:"
                    + Runtime.getRuntime().availableProcessors());

                if (Runtime.getRuntime().availableProcessors() <= 2) {
                    duration = CLICK_FEEDBACK_LESS_DURATION;
                }
                ///M.
